...
원하지 않는 인페인트 결과를 원본으로 되돌리려면 복구 도구를 선택하고 되돌릴 영역을 칠합니다. 기존처럼 **오른쪽 버튼**을 누른 상태에서 인페인팅 도구 또는 글상자 도구를 사용해도 복구할 수 있습니다.  
현재 페이지의 모든 인페인트 결과를 원본으로 되돌리려면 인페인트 지우기 도구 탭의 ```Hard Reset Inpaint``` 버튼을 사용합니다. 이 작업은 실행 취소할 수 있습니다.  
결과는 알고리즘('방법 1' 및 '방법 2'의 GIF)이 텍스트 마스크를 얼마나 정확하게 추출하는지에 따라 달라집니다. 복잡한 텍스트 및 배경에서는 성능이 저하될 수 있습니다.  
'방법 3'은 방법 1의 마스크 검출을 재사용하지만 말풍선 전체를 단색으로 덮지 않고, 주변 문맥과 함께 선택한 인페인터를 항상 실행한 뒤 경계를 부드럽게 합성합니다. 방법 1 결과가 하얀 패치처럼 보일 때 사용하세요.
...
* 텍스트 편집 모드에서 ```Insert``` 또는 우클릭 메뉴의 ```Add text box here``` 를 사용해 현재 캔버스 위치에 텍스트 블록을 추가합니다.
* ```Ctrl+D``` 는 선택한 텍스트 블록을 삭제하고, ```Delete``` 는 선택한 텍스트 블록을 삭제하면서 제거된 이미지 영역을 복구합니다.
* ```P``` 를 눌러 이미지 편집 모드로 전환합니다.  
* 이미지 편집 모드에서 ```J``` 는 인페인트 브러시, ```O``` 는 원본 복구 브러시를 선택합니다.
