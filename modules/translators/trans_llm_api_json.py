...
class LLM_API_Translator(BaseTranslator):
    ...
    def active_api_keys(self) -> List[str]:
        ...
        if self.api_key_tier == "Free":
            ...
            if legacy_keys:
                return legacy_keys
        return get_llm_api_key_pool(self.provider, self.api_key_tier)
    ...
    def _request_translation(self, prompt: str) -> Optional[TranslationResponse]:
        ...
        if self.provider == "LLM Studio":
            ...
            # Google via OpenAI-compat endpoint supports 'json_object'
            self.logger.debug(f"Using 'json_object' mode for {self.provider}.")
            api_args["response_format"] = {"type": "json_object"}

        # Add OpenAI-specific penalties (Google sometimes ignores these but usually accepts them in args)
        ...
        if (
            ...
            json_to_parse = _extract_json(json_to_parse)
            try:
                ...
                try:
                    ...
                    if isinstance(simple_data, dict) and all(
                        ...
                        # 리스트 내부의 딕셔너리에서 'target' 키가 있으면 'translation'으로 변경
                        fixed_translations = []
                        for item in simple_data:
                            if isinstance(item, dict):
                                # target 키를 translation으로 매핑
                                if "target" in item and "translation" not in item:
                                    item["translation"] = item.pop("target")
                            fixed_translations.append(item)
