...
- API 외부 통신, relay/headless 운영, iOS 단축어 연동은 실험적 기능으로 보고 가급적 변경하지 않음. 관련 개선 후보는 `Deferred`로 유지함.
...
| IMP-003 | P1 | Deferred | Ops docs | iOS 단축어 문서에 실제 공개 도메인과 tunnel 이름이 남아 있음 | `doc/IOS_SHORTCUT_KO.md:11`의 `sekai-relay`, `doc/IOS_SHORTCUT_KO.md:19`, `:25`, `:26`, `:139`, `:152`, `:188`의 실제 URL | iOS 단축어 연동은 실험적 기능이므로 현 상태 유지. 공유/배포가 필요해질 때 placeholder 정리 |
| IMP-004 | P1 | Deferred | API hardening | 공개 바인딩 시 인증 토큰이 비어 있어도 서버가 그대로 뜰 수 있음 | `launch.py:45-47`, `relay_server.py:456-460`에서 host와 token을 독립 옵션으로 받고 token 기본값은 빈 문자열. `make_auth_dependency`는 token이 없으면 인증을 생략함 | 외부 API 운영은 실험적 기능이므로 현 상태 유지. 공개 배포를 재개할 때 host/token guard 검토 |
| IMP-005 | P1 | Deferred | API reliability | relay/headless job 상태가 메모리에만 있어 재시작 시 큐/결과 메타데이터가 사라지고, worker 사망 시 `running` job이 stuck 될 수 있음 | `relay_server.py:82-89`, `headless_server.py:223-229`에서 `_jobs = {}`와 in-memory queue만 사용. `relay_server.py:166-174`는 claim 후 lease/timeout reclaim 없음 | relay/headless 외부 처리 흐름은 실험적 기능이므로 현 상태 유지. 장기 운영 전환 시 persistence/reclaim 재검토 |
| IMP-006 | P2 | Deferred | API testing | 업로드 helper 테스트는 있으나 실제 FastAPI endpoint 흐름 테스트가 부족함 | `tests/test_api_uploads.py`는 `RelayJobStore`와 `save_upload_to_project` 중심. `/jobs`, `/translate/raw`, worker result/failure endpoint에 대한 `TestClient` 테스트 없음 | API endpoint 흐름은 실험적 기능이므로 테스트 확장은 보류. 해당 기능을 다시 손볼 때 함께 추가 |
| IMP-007 | P2 | Deferred | Observability | worker와 일부 유틸이 `print` 기반 로그를 사용해 운영 추적성이 낮음 | `local_worker.py:59`, `:74`, `:77`, `:148`이 `print` 사용. `utils/merger.py:218-220`, `:237-255`, `:268-299`도 debug print 사용 | worker 관측성 개선은 외부 통신 작업과 함께 보류. 비API 유틸 로그는 별도 작업에서 필요 시 분리 |
...
1. `IMP-008`로 테스트 실행 방법과 최소 CI 기준을 먼저 안정화합니다.
2. `IMP-009`로 의존성 구성을 분리해 설치 재현성과 속도를 개선합니다.
3. `IMP-010`과 `IMP-011`로 현지화와 OCR 품질 검증을 보강합니다.
4. `IMP-012`-`IMP-013`은 기능 작업과 병행 가능한 구조/품질 개선으로 진행합니다.
