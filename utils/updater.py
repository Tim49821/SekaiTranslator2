...
ALLOW_UPSTREAM_SELF_UPDATE_ENV = 'SEKAI_TRANSLATOR_ALLOW_UPSTREAM_SELF_UPDATE'
SOURCE_UPDATE_DIRS = ('modules', 'ui', 'utils', 'resources')
SOURCE_UPDATE_FILES = ('launch.py', 'requirements.txt')
...
def format_git_update_message(update_branch: str, local_changes_saved: bool, local_branch: str = '') -> str:
    lines = [f'The update was applied on branch: {update_branch}']
...
class BallonsTranslatorUpdater:
    ...
    def apply_update(self, release_info: ReleaseInfo, current_version: str = None) -> UpdateResult:
        ...
        if os.environ.get(ALLOW_UPSTREAM_SELF_UPDATE_ENV, '').strip().lower() not in {'1', 'true', 'yes'}:
            return UpdateResult(
                ...
                release_info=release_info,
                git_message=(
                    'Automatic source replacement is disabled for this flat-layout fork. '
                    f'Set {ALLOW_UPSTREAM_SELF_UPDATE_ENV}=1 only if you intentionally want to apply '
                    'the upstream source zip over the local project.'
                ),
            )
    ...
    def backup_source(self, current_version: str) -> Path:
        ...
        backup_path = self.cache_dir / f'sekaitranslator_{version_tag}'
    ...
    def install_source_zip(self, zip_path: Path) -> None:
        ...
        package_root = release_root / 'ballontranslator'
        self._notify('replace_source', 90, ', '.join(SOURCE_UPDATE_DIRS + SOURCE_UPDATE_FILES))
        for dirname in SOURCE_UPDATE_DIRS:
            source = package_root / dirname
            if dirname == 'resources':
                source = release_root / dirname
            self._replace_directory(source, self.program_path / dirname)
        for filename in SOURCE_UPDATE_FILES:
            source = release_root / filename
            if filename == 'launch.py':
                source = package_root / 'launch.py'
            self._replace_file(source, self.program_path / filename)
        try:
    ...
    def _notify(self, event: str, progress: int, message: str = '') -> None:
        ...
        if self.progress_callback is not None:
            self.progress_callback({'event': event, 'progress': progress, 'message': message})
    ...
    def _on_download_progress(self, payload: dict) -> None:
        ...
        if event == 'file_progress':
            ...
            progress = max(20, min(60, int(downloaded / total * 40) + 20)) if total else 35
