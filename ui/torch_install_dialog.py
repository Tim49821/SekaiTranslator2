...
from utils.torch_install_helper import TORCH_CUDA_VERSION_OPTIONS, TORCH_INSTALL_DEVICE_OPTIONS
...
class TorchInstallHelperDialog(QDialog):
    def __init__(
        ...
        target_row.addWidget(QLabel(self.tr('Device'), self))
        ...
        target_row.addWidget(QLabel(self.tr('CUDA'), self))
        self.cuda_combo = QComboBox(self)
        for cuda_version in TORCH_CUDA_VERSION_OPTIONS:
            self.cuda_combo.addItem(CUDA_VERSION_LABELS.get(cuda_version, cuda_version), cuda_version)
        ...
        layout.addWidget(QLabel(self.tr('Install command'), self))
    ...
    def update_note_text(self):
        device_note = self.tr('NVIDIA: choose CUDA. Intel Arc/Core Ultra: choose XPU. Not sure: choose CPU.')
        if self.selected_device() != 'cuda':
            self.note_text.setText(device_note)
            return
        cuda_note = self.tr('CUDA 12.8 is for RTX 20 / GTX 16 or newer. CUDA 11.8 is for GTX 10 or older.')
        self.note_text.setText(device_note + '\n\n' + cuda_note)
